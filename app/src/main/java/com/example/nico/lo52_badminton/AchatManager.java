...
public class AchatManager {
    private static final String TABLE_NAME = "achat";
    ...
    public static final String CREATE_TABLE_ACHAT = "CREATE TABLE "+TABLE_NAME+
            ...
            " "+KEY_DATE_ACHAT+" TEXT," +
            " primary key ("+KEY_ID_CLIENT+","+KEY_ID_PRODUIT+")"+
            ");";
    ...
    public int modAchat(Achat achat) {
        ...
        String where = KEY_ID_CLIENT+" = ? and "+KEY_ID_PRODUIT+" = ?";
        String[] whereArgs = {achat.getId_client()+"",achat.getId_produit()+""};
    ...
    public Achat getAchat(int idCl,int idProd) {
        // Retourne l'Achat dont l'id est passé en paramètre

        Achat a=new Achat(0,0,0,0,"");

        Cursor c = db.rawQuery("SELECT * FROM "+TABLE_NAME+" WHERE "+KEY_ID_CLIENT+"="+idCl+" AND "+KEY_ID_PRODUIT+"="+idProd, null);
        if (c.moveToFirst()) {
